...
При запуске установщика Windows покажет синее окно «Windows защитила ваш
компьютер». Это ожидаемо, и вот почему.

Чтобы такого окна не было, установщик подписывают сертификатом
разработчика. Сертификат стоит денег и выдаётся организации или ИП.
prettyeyes - личный проект без публичной лицензии, он не продаётся и не
распространяется как продукт, поэтому сертификата у него нет. SmartScreen не
знает эту подпись и предупреждает обо всём, что видит впервые.

Поставить всё равно: **Подробнее** - **Выполнить в любом случае**.

Если файл скачан браузером, Windows могла дополнительно пометить его как
пришедший из интернета. Тогда правой кнопкой по файлу - **Свойства** - внизу
галочка **Разблокировать** - **ОК**.

Не доверять на слово - нормально. Исходники здесь целиком, сборка делается
одной командой из раздела ниже, а SHA-256 установщика печатается в конце сборки
и указан в описании каждого релиза.
